...
public class AmazonSortCenter {
	...
	public static void main(String[] args) {
		int[] packagesCopy = {1, 10, 25, 35, 60};
		List list = Arrays.stream(packagesCopy).boxed().collect(Collectors.toList());	
		int space = 90;
		boolean found = false;
		Arrays.sort(packagesCopy);
		int maxPackageWeight = 0, index1 = -1,index2 = -1;
		for(int i =  packagesCopy.length -1 ; i >=0 && !found ; i--) {
			maxPackageWeight = packagesCopy[i];
			for(int j=packagesCopy.length-1;j > 0; j--) {
				if((maxPackageWeight + packagesCopy[j]) <= space -30 && i!=j) {
					index1 = list.indexOf(packagesCopy[j]);
					index2 = list.indexOf(packagesCopy[i]);
					found = true;
					break;
				}
			}			
